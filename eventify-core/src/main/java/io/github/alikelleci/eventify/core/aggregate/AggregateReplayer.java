 ...
 * order they were handled (see {@link MessageIds#nextEventKey}). That range can also hold another aggregate's events (see {@link MessageIds#isKeyOf}): those are
...
public class AggregateReplayer {
  ...
  public Result replay(ReadOnlyKeyValueStore<String, Event> eventStore, String aggregateId, AggregateState start,
    ...
    if (untilEventId != null && !MessageIds.isKeyOf(aggregateId, untilEventId)) {
    ...
    if (start != null && !MessageIds.isKeyOf(aggregateId, start.getEventId())) {
    ...
    String from = start != null ? start.getEventId() + "\0" : MessageIds.firstKey(aggregateId); // after the starting state's event
