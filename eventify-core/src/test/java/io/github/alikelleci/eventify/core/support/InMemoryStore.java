...
public class InMemoryStore<V> implements ReadOnlyKeyValueStore<String, V> {
  ...
  private static <V> KeyValueIterator<String, V> iterator(Map<String, V> map) {
    Iterator<Map.Entry<String, V>> iterator = Map.copyOf(map).entrySet().stream()
        .sorted(Map.Entry.comparingByKey()).iterator();
